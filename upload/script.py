...
def _extract_run(run, start_position):
  ...
  return {
      KEY_VALUE: run_text,
      KEY_TYPE: run_type,
      "start_position": start_position  # Adding start position to the run data
  }
      ...
      for _, para in enumerate(page[KEY_PARAGRAPHS]):
        ...
        for run in para[KEY_RUNS]:
          if run[KEY_TYPE] == KEY_BOLD:
            text += f"<b>{run[KEY_VALUE]}</b>"
          elif run[KEY_TYPE] == KEY_ITALIC:
            text += f"<i>{run[KEY_VALUE]}</i>"
          elif run[KEY_TYPE] == KEY_UNDERLINE:
            text += f"<u>{run[KEY_VALUE]}</u>"
          elif run[KEY_TYPE] == KEY_STRIKE:
            text += f"<s>{run[KEY_VALUE]}</s>"
          else:
            text += f"{run[KEY_VALUE]}"
        text += "</p>\n"
...
def count_rows_in_csv(csv_file, required_columns):
  with open(csv_file, mode='r', newline='') as file:
    ...
    required_indices = [headers.index(col)
                        for col in required_columns if col in headers]
    ...
    for row in reader:
      ...
      if all(row[idx].strip() for idx in required_indices):
        valid_row_count += 1
...
def exec(exec_upload, prune, direct_path, limit, upload_config):
  ...
  total_letters = count_rows_in_csv(os.path.join(
    cur_path, os.pardir, 'BG to BB Letters_Spreadsheet - Sheet1.csv'), ['Letter_ID'])
  ...
  for letter_id, row in letters_df.iterrows():
    try:
      ...
      parsed_letter = next(
        (item for item in parsed_letters if item[KEY_ID] == letter_id), None)
